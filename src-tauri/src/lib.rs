            ...
            if let Some(v) = current_version {
                ...
                for removal in removals {
                    if let Some(target_oses) = removal.on() {
                        ...
                        if cfg!(target_os = "macos") && cfg!(target_arch = "aarch64") && !target_oses.contains(&"mac".to_string()) {
                            continue;
                        }
                        if cfg!(target_os = "macos") && cfg!(target_arch = "x86_64") && !target_oses.contains(&"mac-intel".to_string()) {
                            continue;
                        }
